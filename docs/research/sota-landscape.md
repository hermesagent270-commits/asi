...
| [TeLAPA](https://arxiv.org/abs/2604.15414v1) | Preserves per-task policy neighborhoods and maintains a shared learned trajectory latent for later retrieval and adaptation. | The [bounded qualification smoke](../runbooks/telapa-qualification.md) binds the public code at commit `a4dc16e…` and exercises only ASI's archive primitive in SwitchingTwoState. The repository's declared license file is absent, and the paper environments, PPO/MAP-Elites, and learned trajectory embedder remain unimplemented, so paper parity stays closed. |
